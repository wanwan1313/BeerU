...
$e_sid = isset($_GET['event_sid']) ? intval($_GET['event_sid']):NULL;
    ...
    switch( $action ){
        case 'add' :
            if( !empty($e_sid)){
                ...
                (`member_sid`, `event_sid`,`created_at`) 
                VALUES (?,?,NOW())";
        ...
        case 'delete' :
            if( !empty($e_sid)){
                //從attention資料表裡刪資料
                $d_SQL= " DELETE FROM `attention` WHERE `member_sid`=? AND `event_sid`=?";
